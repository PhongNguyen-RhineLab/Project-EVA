"""
Comprehensive Evaluation Script for EVA Project (UMAP Version)
Evaluates trained model on test set with UMAP visualization
"""
...
def plot_latent_space_umap(latents, labels, save_path, n_neighbors=15, min_dist=0.1):
    """
    Visualize latent space using UMAP

    Args:
        latents: Latent representations (n_samples, latent_dim)
        labels: Multi-label emotion vectors (n_samples, n_emotions)
    ...
    try:
        ...
        print("⚠️  UMAP not installed. Installing...")
        import subprocess
    ...
    # Plot 1: Scatter plot with emotion classes
    for i, emotion in enumerate(EMOTION_LABELS):
    ...
    ax1.set_title('Latent Space Visualization (UMAP) - By Dominant Emotion', fontsize=14, fontweight='bold')
    ...
    idx = z.argsort()
    x, y, z = latents_2d[idx, 0], latents_2d[idx, 1], z[idx]

    scatter = ax2.scatter(x, y, c=z, s=30, cmap='viridis', alpha=0.6)
    ...
    ax2.set_title('Latent Space Visualization (UMAP) - Density Map', fontsize=14, fontweight='bold')
    ax2.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"✓ Saved UMAP latent space visualization to {save_path}")


def plot_latent_space_umap_multi(latents, labels, save_path):
    """
    ...
    print("Computing multiple UMAP projections...")

    # Different parameter combinations
    param_configs = [
        {'n_neighbors': 5, 'min_dist': 0.0, 'title': 'Local Structure (n=5, dist=0.0)'},
        {'n_neighbors': 15, 'min_dist': 0.1, 'title': 'Balanced (n=15, dist=0.1)'},
        {'n_neighbors': 50, 'min_dist': 0.5, 'title': 'Global Structure (n=50, dist=0.5)'},
        {'n_neighbors': 30, 'min_dist': 0.99, 'title': 'Very Global (n=30, dist=0.99)'}
    ]
    ...
    for idx, config in enumerate(param_configs):
        print(f"  Computing projection {idx + 1}/4...")

        # Create UMAP with specific parameters
        reducer = umap.UMAP(
    ...
    plt.suptitle('UMAP Latent Space - Different Parameter Settings',
                 fontsize=16, fontweight='bold', y=1.00)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"✓ Saved multi-parameter UMAP visualization to {save_path}")
...
def main():
    ...
    print("=" * 70)
    print("EVA PROJECT - MODEL EVALUATION (UMAP Version)")
    print("=" * 70)
    print(f"\nDevice: {DEVICE}")
    ...
    plot_latent_space_umap(
        latents, y_true,
        save_path=os.path.join(OUTPUT_DIR, 'latent_space_umap.png')
    )

    plot_latent_space_umap_multi(
        latents, y_true,
        save_path=os.path.join(OUTPUT_DIR, 'latent_space_umap_multi.png')
    )
    ...
    print(f"  • latent_space_umap.png - UMAP visualization (single)")
    print(f"  • latent_space_umap_multi.png - UMAP with multiple parameters")
